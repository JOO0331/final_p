...
def load_steam_tag_indie():
    with open('game_list/steam_tag_indie.json', 'r', encoding='utf-8') as f:
        return json.load(f)
        ...
        if details_response.status_code == 200:
            ...
            if str(app_id) in data and data[str(app_id)]['success']:
                ...
                if game_data.get('release_date', {}).get('date'):
                    try:
                        release_date = datetime.strptime(game_data['release_date']['date'], '%d %b, %Y')
                    except ValueError:
                        release_date = datetime.strptime(game_data['release_date']['date'], '%b %Y')
                else:
                    release_date = None
...
def fetch_and_save_game_reviews(game):
    ...
    print(f"게임 {game.name}({game.app_id})의 리뷰 가져오기 시작")
    
    reviews_response = requests.get(reviews_url)
    if reviews_response.status_code == 200:
        reviews_data = reviews_response.json()
        reviews = reviews_data.get('reviews', [])
        print(f"가져온 리뷰 수: {len(reviews)}")

        if not reviews:
            print(f"게임 {game.name}({game.app_id})에 대한 리뷰를 찾을 수 없습니다.")
            return
        ...
        for review in reviews[:5]:
            GameReview.objects.update_or_create(
                ...
                defaults={
                    ...
                    'created_at': timezone.make_aware(datetime.fromtimestamp(review['timestamp_created']))
                }
            )
